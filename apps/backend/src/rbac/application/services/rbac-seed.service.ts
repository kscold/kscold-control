...
export class RbacSeedService {
  ...
  async seedInitialData(): Promise<void> {
    const permissionDescriptions: Record<string, string> = {
      [PERMISSIONS.DASHBOARD_READ]: '대시보드 운영 요약 조회',
    ...
    const readOnlyPerms = allPermissions.filter(
      (p) =>
        p.name === PERMISSIONS.DASHBOARD_READ ||
    ...
    if (!readOnlyRole) {
      readOnlyRole = this.roleRepository.create({
        name: ROLES.READ_ONLY,
        description: '읽기 전용',
        permissions: readOnlyPerms,
      });
    } else {
      readOnlyRole.permissions = readOnlyPerms;
    }
    ...
    if (!operatorRole) {
      ...
      operatorRole.permissions = operatorPerms;
    }
    await this.roleRepository.save(operatorRole);

    // Create Terminal Only role
    const terminalPerms = allPermissions.filter(
      (p) => p.name === PERMISSIONS.TERMINAL_ACCESS,
    );
    let terminalRole = await this.roleRepository.findByNameWithPermissions(
      ROLES.TERMINAL_ONLY,
    );
    if (!terminalRole) {
      terminalRole = this.roleRepository.create({
        name: ROLES.TERMINAL_ONLY,
        description: '터미널 접근만 가능',
        permissions: terminalPerms,
      });
    } else {
      terminalRole.permissions = terminalPerms;
    }
    await this.roleRepository.save(terminalRole);
    ...
    let guestRole = await this.roleRepository.findByNameWithPermissions(
      ROLES.GUEST,
    );
    if (!guestRole) {
      guestRole = this.roleRepository.create({
        name: ROLES.GUEST,
        description: '구경 전용 (읽기만 가능, 터미널 10회 제한)',
        permissions: guestPerms,
      });
    } else {
      guestRole.permissions = guestPerms;
    }
    await this.roleRepository.save(guestRole);
